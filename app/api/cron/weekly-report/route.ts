...
import { getResend, buildWeeklyReportEmail, type PeriodReportDay } from "@/lib/resend";
...
export async function GET(req: Request) {
  ...
  // Period: previous Sun-Sat (assumes this cron runs Sunday morning).
  const todayDate = new Date();
  const endDate = new Date(todayDate);
  endDate.setDate(endDate.getDate() - 1); // Saturday
  ...
  for (const email of allowedEmails) {
    ...
    const emailPayload = buildWeeklyReportEmail({
      toEmail: email,
      periodLabel: "שבועי",
      periodStart,
      periodEnd,
